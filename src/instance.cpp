...
vector<Remorque*>* Instance::getRemorques() {
    return this->remorques;
}

vector<Station*>* Instance::getStations() {
...
// On suppose que tous les sites (Remorques ou Stations) sont créés.
//
// Rappel :
//   Le champs sid est unique pour un Site donc une Remorques et une Station ne
//   peuvent jamais avoir le même sid.
//
...
void Instance::build_arcs() {
    ...
    // Log::level += 5;
    int nb_sites = this->get_nb_sites();
    // cout << "xxxxx nb_sites=" << nb_sites <<"\n";
    ...
    for (int i1=0; i1 < nb_sites; i1++) {
        ...
        for (int i2=0; i2 < nb_sites; i2++) {
            ...
            arcs_grid->at(nb_sites*site1->id + site2->id) = arc;
            // (*arcs_map)[i1*nb_sites + i2] = arc;
            // dists_grid->at(nb_sites*site1->sid + site2->sid) = arc->dist;
            dists_grid->at(nb_sites*site1->id + site2->id) = arc->dist;
            if (arc->is_feasible()) {
                arcs->push_back(arc);
            }
        }
...
Arc* Instance::get_arc(int i1, int i2) {
    ...
    //     exit(1);
    // };
    return (*arcs_grid)[i1 * this->get_nb_sites() + i2];
}

Arc* Instance::get_arc(Site* s1, Site* s2) {
    // return get_arc(s1->sid, s2->sid);
    return get_arc(s1->id, s2->id);
}
...
Station* Instance::find_station(string name) {
...
Instance* Instance::new_velib_mini() {
    ...
    //   stations->sort(Site::compareByName);
    //
